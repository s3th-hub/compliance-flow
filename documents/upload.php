...
if (!$compliance_id) { header('Location: ' . BASE_URL . '/compliance/records.php'); exit; }
...
$stmt->close();
if (!$compliance) { header('Location: ' . BASE_URL . '/compliance/records.php'); exit; }
...
if (isset($_GET['del']) && (int)$_GET['del'] > 0) {
    ...
    if ($doc) {
        ...
        $db->prepare('DELETE FROM documents WHERE id = ?')->execute() || $db->query("DELETE FROM documents WHERE id = $doc_id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['document'])) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        $stored_name = uniqid('doc_', true) . '.' . strtolower($ext);
        ...
        if (move_uploaded_file($file['tmp_name'], $dest)) {
            ...
            $uid  = $user['id'];
            $stmt = $db->prepare(
                'INSERT INTO documents (compliance_id, original_name, stored_name, file_type, file_size, uploaded_by)
                 VALUES (?,?,?,?,?,?)'
            );
...
// Load docs
$docs = $db->prepare('SELECT d.*, u.full_name FROM documents d JOIN users u ON u.id = d.uploaded_by WHERE d.compliance_id = ? ORDER BY d.uploaded_at DESC');
$docs->bind_param('i', $compliance_id);
$docs->execute();
$documents = $docs->get_result()->fetch_all(MYSQLI_ASSOC);
$docs->close();

if (isset($_GET['success'])) $success = $_GET['success'] === 'deleted' ? 'Document deleted.' : 'Document saved.';
...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="<?= BASE_URL ?>/compliance/records.php">Compliance</a></li>
...
<div class="d-flex justify-content-between align-items-start mb-3">
    <div>
        ...
        <p class="text-muted mb-0"><?= clean($compliance['business_name']) ?> &middot; Expires: <?= date('d M Y', strtotime($compliance['expiry_date'])) ?></p>
    </div>
    <span class="badge badge-<?= $compliance['status'] ?> fs-6"><?= str_replace('_', ' ', ucfirst($compliance['status'])) ?></span>
</div>
...
<div class="row g-3">
    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-header fw-semibold"><i class="bi bi-cloud-upload me-2"></i>Upload Document</div>
            <div class="card-body">
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header fw-semibold"><i class="bi bi-folder2-open me-2"></i>Uploaded Documents (<?= count($documents) ?>)</div>
            <div class="card-body p-0">
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item d-flex align-items-center gap-3 py-2">
                        <i class="bi bi-<?= str_contains($doc['file_type'], 'pdf') ? 'file-earmark-pdf text-danger' : 'file-earmark-image text-primary' ?> fs-4"></i>
                        <div class="flex-grow-1">
                            <div class="fw-semibold small"><?= clean($doc['original_name']) ?></div>
                            <div class="text-muted" style="font-size:.78rem;">
                                <?= round($doc['file_size'] / 1024) ?>KB &middot;
                                Uploaded by <?= clean($doc['full_name']) ?> &middot;
                                <?= date('d M Y H:i', strtotime($doc['uploaded_at'])) ?>
                            </div>
                        </div>
                        <div class="d-flex gap-1">
                            <a href="<?= BASE_URL ?>/documents/view.php?id=<?= $doc['id'] ?>" class="btn btn-sm btn-outline-primary" target="_blank" title="View">
                                <i class="bi bi-eye"></i>
                            </a>
